...
bool SpodesClient::ConnectBleTransport()
{
    ...
    // [Android-патч] max_length_ обычно выставляется в ReceiveUnnumberedAcknowledge()
    // как часть SetNormalResponseMode() — шага согласования HDLC-параметров для
    // последовательного порта, которого в BLE-сценарии нет (см. ConnectSerialPort
    // выше). Без этого max_length_ остаётся 0, и ReadResponse() читает 0 байт из
    // транспорта МГНОВЕННО (условие "rx_buffer_.size() >= 0" всегда истинно), ещё
    // до реального обмена по BLE — EstablishConnectionResponse() тут же падает с
    // "невалидный ответ". Задаём тот же дефолт, что использует
    // ReceiveUnnumberedAcknowledge() при отсутствии согласованных параметров.
    max_length_ = 12 + 256;
...
// [Android-патч] см. комментарий в spodes_client.h — этот метод не переиспользует
// FormHDLCHeader/AddHDLCHeader (те всегда пишут двухбайтовый logical+physical адрес),
// а собирает HDLC-заголовок вручную с ОДНИМ байтом адреса назначения/источника — именно
// так делает прошивка пульта РиМ 040.40 (см. HDLC_Wrapper_Preset в её исходниках).
// LLC/APDU-часть кадра по-прежнему собирается штатными функциями библиотеки
// (FormLLCHeader/AddAddress) — это всё ещё нормальный DLMS/COSEM GET-request,
...
bool SpodesClient::GetResponseRaw (std::vector<uint8_t> &response)
{
    ...
    {
        error_message_ = "SpodesClient::GetResponseRaw: " + (std::string)ex.what();
        return false;
    }
}

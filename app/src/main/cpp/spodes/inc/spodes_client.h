...
public:
    ...
    /**
     * @brief [Android-патч] SNRM, собранный СТРОГО по стандарту — для приборов, которые этот кадр
     *        реально обрабатывают и отвечают UA (проверено на счётчике AKROS).
     *
     * ЗАЧЕМ ОТДЕЛЬНЫЙ МЕТОД. Кадр, который собирает SetNormalResponseMode() выше, оказался
     * испорчен сразу в трёх местах (сверено с эталонным логом обмена штатной утилиты и
     * пересчётом контрольных сумм):
     *   1. в информационное поле U-кадра вставляется LLC-заголовок E6 E6 00 (см.
     *      FormSNRMInfromationField) — в SNRM его быть не должно, там сразу идёт формат 0x81;
     *   2. поле длины кадра считается «магическим» += 21/23 от захардкоженного 0x08 и в
     *      результате не совпадает с реальной длиной (29 вместо 34);
     *   3. FCS считается по диапазону, в котором ещё нет места под саму FCS, из-за чего
     *      отбрасываются два последних байта информационного поля и сумма не сходится.
     *
     * Приборы РиМ этого не замечали, потому что на SNRM они не отвечают вовсе — кадр улетал в
     * эфир и никого не смущал. AKROS же кадр обрабатывает, видит несошедшуюся FCS и молча его
     * отбрасывает: в логе это выглядело как таймаут ожидания UA.
     *
     * Старый метод НЕ тронут намеренно: он продолжает отправлять ровно те байты, что и раньше,
     * поэтому поведение с приборами РиМ не меняется ни на бит. Новый используется только там,
     * где UA действительно ждут.
     *
     * Собранный кадр побайтно совпадает с эталонным SNRM из лога рабочей утилиты.
     *
     * @param addr - логический и физический адреса сервера, идентификатор клиента
     * @param max_info_transmit - максимальный размер информационного поля на передачу
     * @param max_info_receive - максимальный размер информационного поля на приём
     * @return - true в случае успешного выполнения метода, иначе false
     */
    bool SetNormalResponseModeStandard(const ConnectionAddresses &addr,
                                       const uint16_t &max_info_transmit,
                                       const uint16_t &max_info_receive);

    /**
     * @brief [Android-патч] Устанавливает ШИФРОВАННУЮ (высокоуровневую) ассоциацию DLMS/COSEM с
     * прибором AKROS — HLS с механизмом GMAC (2.16.756.5.8.2.5), защита suite 0 (AES-128-GCM).
     *
     * ЗАЧЕМ ОТДЕЛЬНЫЙ МЕТОД. Штатный EstablishConnectionRequest()/Response() библиотеки умеет
     * шифрование лишь наполовину и под ДРУГОЙ механизм: ответ на HLS-вызов он считает как
     * AES-ECB(StoC) (это механизм 4), тогда как AKROS ждёт GMAC-тег (механизм 5); байт контроля
     * безопасности выставляется десятичными 10/20 вместо 0x10/0x20; счётчик вызовов нигде не
     * ведётся; обычный GET вообще не шифруется. Здесь весь обмен собран заново и ПОБАЙТОВО сверен
     * с логом реального приложения (см. spodes_ciphered.hpp и офлайн-тест).
     *
     * ПОСЛЕДОВАТЕЛЬНОСТЬ (после уже установленного SNRM/UA, см. SetNormalResponseModeStandard):
     *   1. AARQ (контекст 2.16.756.5.8.1.3, механизм 5, случайный вызов CtoS, зашифрованный
     *      glo-initiate-request) → AARE (проверяем «принято», достаём system-title сервера и StoC);
     *   2. ACTION reply_to_HLS_authentication (метод 1 объекта Association LN, класс 15,
     *      0.0.40.0.0.255) со значением SC||IC||GMAC(StoC) — зашифрованный glo-action-request →
     *      glo-action-response (проверяем успех).
     * После успеха все GET шифруются автоматически (см. ciphered_session_).
     *
     * @param key - ключ (он же пароль) — ровно 16 байт ASCII, напр. "SettingRiM_AKROS"
     * @param key_len - длина ключа (должна быть 16)
     * @return - true, если ассоциация установлена и HLS-аутентификация подтверждена
     */
    bool EstablishCipheredConnection(const char* key, uint8_t key_len);

    /**
     * @brief [Android-патч] GET-запрос внутри шифрованной ассоциации (см.
     * EstablishCipheredConnection): собирает обычный GET-request-normal, оборачивает его в
     * glo-get-request (SC=0x30) со следующим значением счётчика вызовов и шлёт как I-кадр со
     * штатной (двухбайтовой) адресацией. Ответ читается и расшифровывается в GetResponseRaw().
     * @param params - класс, OBIS-код, атрибут
     * @return - true, если кадр собран и отправлен
     */
    bool GetRequestCiphered(const RequestParams &params);
    ...
    /**
     * @brief [Android-патч] Публичная обёртка над SendReadyToReceive() — запрос СЛЕДУЮЩЕГО
     * блока ответа, разбитого прибором на части (get-request-next, DLMS-сервис 0xC0 0x02).
     *
     * Зачем понадобилась. Сам SendReadyToReceive() объявлен в приватной секции: библиотека
     * рассчитана на то, что блочную передачу целиком отрабатывают её собственные
     * GetResponseInt()/GetResponseFloat()/GetResponseArray() — они читают ответ, видят «блок
     * не последний» и дозапрашивают остальные сами. Но на этом приборе разбор DLMS вынесен на
     * сторону Kotlin (см. GetResponseRaw() выше и MeterRepositoryImpl.kt): нативный код отдаёт
     * «сырые» байты, а решение «это блочный ответ, нужен следующий блок» принимается там же,
     * где разбирается APDU. Значит, и запрашивать следующий блок должен вызывающий код.
     *
     * Сделано именно обёрткой, а не переносом метода в public: приватность SendReadyToReceive()
     * — часть исходной библиотеки, и менять её состав ради одного вызова незачем. Логика
     * запроса (номера последовательности HDLC, HCS, FCS) остаётся единственной и неизменной.
     *
     * @param block_number - номер УЖЕ ПРИНЯТОГО блока (по стандарту в get-request-next
     *        указывается именно он, а не номер запрашиваемого)
     * @return - true, если кадр успешно собран и отправлен
     */
    bool SendReadyToReceiveBlock(const int8_t &block_number)
    {
        return SendReadyToReceive(block_number);
    }
...
private:
    // [Android-патч] Транспортные помощники шифрованной ассоциации (см. EstablishCipheredConnection).
    // SendApduFrame — обернуть готовый APDU (байты после E6 E6 00) в I-кадр со штатным HDLC-
    // заголовком и отправить; ReadApduFrame — прочитать кадр и вернуть его APDU (между E6 E7 00 и
    // хвостом FCS+флаг).
    bool SendApduFrame(const std::vector<uint8_t> &apdu);
    std::vector<uint8_t> ReadApduFrame();
    ...
    // [Android-патч] Состояние ШИФРОВАННОЙ ассоциации AKROS (см. EstablishCipheredConnection).
    // ciphered_session_ включается после успешной HLS-аутентификации; пока он true, GET
    // шифруются (GetRequestCiphered), а ответы расшифровываются в GetResponseRaw(). Единый
    // счётчик вызовов invocation_counter_ инкрементируется на КАЖДУЮ операцию GCM (кадр или
    // GMAC ответа на HLS) и кодируется в поле IC младшим байтом вперёд. akros_ciph_key_ — ключ
    // (он же ключ аутентификации), akros_client_sys_title_ — system-title клиента (calling-AP-
    // title, используется в IV); оба фиксированы, как у реального приложения.
    bool ciphered_session_ = false;
    uint32_t invocation_counter_ = 0;
    uint8_t akros_ciph_key_[16] = {};
    uint8_t akros_client_sys_title_[8] = {0x52,0x4D,0x53,0x3E,0x30,0x2E,0x22,0x5C};
